...
def train():
    # Menggunakan kustom basic environment
    env = gym.make('gymnasium_env/MLAgentsBasic-v0')
    ...
    policy_net = BasicDQN(n_observations, n_actions).to(device)
    target_net = BasicDQN(n_observations, n_actions).to(device)
    ...
    print("\nMemulai training agen DQN pada MLAgentsBasic-v0...\n")
    ...
    for i_episode in range(num_episodes):
        ...
        # Progress status
        if (i_episode + 1) % 10 == 0 or (i_episode + 1) == num_episodes:
    ...
    torch.save(policy_net.state_dict(), "basic_dqn_model.pth")
    print("Model disimpan sebagai 'basic_dqn_model.pth'\n")
    env.close()
